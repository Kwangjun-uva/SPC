...
class AdEx_Layer(object):
    ...
    def __call__(self, sim_duration, time_step, lt, I_ext, bat_size):
        ...
        #         self.Iext = tf.constant(
        #             I_ext[:, int(gpu_i * len(I_ext.T) / len(self.gpus)):int((gpu_i + 1) * len(I_ext.T))],
        #             dtype=tf.float32)
        ...
        self.xtr_record = self.xtr_record / (self.l_time / self.dt)
...
def save_data(sim_name):
    ...
    sim_params = {'n_pc_layers': n_pc_layers, 'n_pred_neurons': n_pred_neurons, 'n_gist': n_gist,
                  'batch_size': batch_size, 'n_shape': n_shape, 'sim_dur': sim_dur, 'dt': dt,
